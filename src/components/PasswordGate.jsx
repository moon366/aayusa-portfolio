    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 relative overflow-hidden px-6">
      ...
      >
        <div className="flex items-center justify-center gap-4 mb-6">
          <SunflowerIcon className="w-12 h-12 md:w-14 md:h-14" />
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-br from-white via-slate-200 to-slate-500 bg-clip-text text-transparent">
            Ankita Miss
          </h1>
          <RabbitIcon className="w-12 h-12 md:w-14 md:h-14" />
        </div>
        <p className="text-base md:text-lg text-cyan-400 mb-10">
          This is a secret page. Enter the passcode to continue...
        </p>

        <form
          onSubmit={handleSubmit}
          className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6 shadow-xl backdrop-blur"
        >
          <input
            type="password"
            value={password}
            ...
            autoFocus
            className="w-full px-4 py-3 mb-4 rounded-lg bg-slate-900 border border-slate-600 text-slate-100 placeholder-slate-500 outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 transition-all"
          />
          <motion.button
            type="submit"
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
            className="w-full px-6 py-3 bg-cyan-500 text-slate-900 font-semibold rounded-lg shadow-lg hover:bg-cyan-400 transition-all duration-300 cursor-pointer"
          >
            Unlock
          </motion.button>
          {error && (
            <p className="mt-4 text-sm text-red-400">Incorrect passcode. Try again.</p>
          )}
        </form>
      </motion.div>
    </div>
